...
const SLIDES = [
  {
    title: <>Your Story,<br /> Your Sanctuary</>,
  ...
  {
    title: <>Write Boundlessly,<br /> Everywhere</>,
    ...
    image: img2 // Заміни на img2
  },
  {
    title: <>Control<br /> Every Idea</>,
...
const LandingPage = () => {
  ...
  const [isPaused, setIsPaused] = useState(false); // НОВЕ: Стан паузи
  ...
  // Професійний автоплей: враховує паузу і скидається при ручному перемиканні
  useEffect(() => {
    if (isPaused) return; // Якщо мишка на слайді - не перемикаємо!

    const timer = setInterval(() => {
      changeSlide('next');
    }, 12000); // 12 секунд для комфортного читання

    // currentSlide в залежностях гарантує, що при ручному кліку таймер почнеться з нуля
    return () => clearInterval(timer);
  }, [isPaused, currentSlide, isFading]); 

  // Логіка для свайпів + Пауза при дотику
  const handleTouchStart = (e: React.TouchEvent) => {
    setTouchStart(e.targetTouches[0].clientX);
    setIsPaused(true); // Зупиняємо таймер, поки палець на екрані
  };
  const handleTouchMove = (e: React.TouchEvent) => setTouchEnd(e.targetTouches[0].clientX);
  const handleTouchEnd = () => {
    setIsPaused(false); // Відпускаємо палець - таймер йде далі
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-[#F6F3E9] font-serif text-[#4A0404] overflow-hidden">
      ...
      <main 
        ...
        onMouseEnter={() => setIsPaused(true)}   /* Пауза на десктопі */
        onMouseLeave={() => setIsPaused(false)}  /* Відновлення на десктопі */
      >
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left flex-grow justify-center relative">
          ...
          <h1 className={`text-5xl md:text-7xl font-normal leading-[1.1] mb-6 md:mb-8 text-[#4A0404] ${fadeClass}`}>
          ...
          <p className={`text-base md:text-lg leading-relaxed text-[#1A1A1A]/70 mb-8 md:mb-10 font-sans font-normal max-w-xl ${fadeClass}`}>
        ...
        {/* Індикатор крапок (Тільки для мобільних) */}
        <div className="absolute bottom-1 left-0 w-full flex justify-center gap-2 lg:hidden z-10">
          {SLIDES.map((_, index) => (
            <button
              key={index}
